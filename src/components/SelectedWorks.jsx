...
function ExploreTinkering({ scrollYProgress }) {
	...
	const pointerEvents = useTransform(scrollYProgress, v => (v > 0.86 && v < 0.96) ? "auto" : "none");

	return (
		<motion.div style={{ opacity: deskOpacity }} className="absolute inset-0 w-full h-full bg-[#D4C5B0] flex items-center justify-center z-40 overflow-hidden">
			...
			<ScatterItem scrollYProgress={scrollYProgress} scatterProgress={scatterProgress} xVals={["-100vw", "-250px", "-100vw"]} yVals={["0vh", "-140px", "0vh"]} rVals={[-90, -15, -90]}>
				<CoffeeMug />
			</ScatterItem>
			...
			<motion.a href="/work" style={{ opacity: textOpacity, pointerEvents }} className="absolute z-50 flex flex-col items-center justify-center group cursor-pointer text-white">
...
export default function SelectedWorks() {
	...
	const titleOpacity = useTransform(scrollYProgress, [0.60, 0.66], [1, 0]);
