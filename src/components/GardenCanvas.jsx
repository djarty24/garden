...
export default function GardenCanvas() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const flowersData = [];
            snapshot.forEach((doc) => flowersData.push({ id: doc.id, ...doc.data() }));
    ...
    const handlePlantClick = async () => {
        ...
        try {
            ...
            setCurrentPage(1);
            setName(''); setMessage(''); setSong(''); setWebsite('');
        } catch (error) {
        ...
        <section className="w-full min-h-screen bg-canvas relative flex flex-col pt-24 pb-40 px-4 md:px-8 z-10">
            ...
            <div className="text-center z-10 mb-8 relative flex flex-col items-center">
                ...
                <p className="text-xs md:text-sm text-slate font-sans max-w-md mx-auto mb-5">
                    ...
                    <span className="block mt-2 text-[10px] md:text-xs text-slate/70">
                ...
                <p className="text-[10px] md:text-xs font-serif italic text-slate/80">{plantedFlowers.length} flowers have been planted in my garden!</p>
            </div>
            ...
            <div className="w-full max-w-6xl mx-auto grow flex flex-col items-center justify-start z-10">
                    ...
                    {currentFlowers.map((flower) => {
                        ...
                        return (
                            <div key={flower.id} className="relative flex flex-col items-center justify-center aspect-square border border-slate/10 bg-slate/5 rounded-xl group hover:bg-[#FDF1F3]/50 hover:border-[#c88d9a]/30 transition-all duration-300">
                                <motion.div layout={false} className="relative w-full h-full flex items-center justify-center cursor-pointer" animate={{ rotate: [-2, 2, -2] }} transition={{ repeat: Infinity, duration: 4 + (flower.id % 3), ease: "easeInOut" }} onClick={() => setActiveMessage(activeMessage === flower.id ? null : flower.id)}>
                                ...
                                <AnimatePresence>
                                    {activeMessage === flower.id && (
                                        <motion.div layout={false} initial={{ opacity: 0, y: 5, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 5, scale: 0.95 }} className="absolute top-[105%] left-1/2 -translate-x-1/2 mt-2 w-48 sm:w-56 bg-white p-4 rounded-xl shadow-xl border border-slate/10 z-50 text-left" onClick={(e) => e.stopPropagation()}>
                                            <p className="font-serif italic text-ink text-sm mb-3">"{flower.message}"</p>
                                            {flower.song && (
                                                <p className="text-[10px] font-sans text-slate mb-2 flex items-center gap-2">
                                                    <svg className="w-3 h-3 text-[#c88d9a] shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg>
                                                    <span className="truncate">{flower.song}</span>
                                                </p>
                                            )}
                ...
                {totalPages > 1 && (
                    <div className="mt-16 flex items-center gap-8 font-sans text-xs tracking-widest text-slate">
                        <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className={`hover:text-ink transition-colors ${currentPage === 1 ? 'opacity-30 cursor-not-allowed' : ''}`}>PREV</button>
                        <span>{currentPage} / {totalPages}</span>
                        <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className={`hover:text-ink transition-colors ${currentPage === totalPages ? 'opacity-30 cursor-not-allowed' : ''}`}>NEXT</button>
                    </div>
                )}
            </div>
                        ...
                        <div className="p-6 h-full flex flex-col">
                            ...
                            <div className="flex flex-col gap-4 grow">
                                ...
                                <div>
                                    ...
                                    <textarea value={message} onChange={(e) => setMessage(e.target.value)} className="w-full bg-[#FDF1F3] text-ink px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#c88d9a] font-sans resize-none h-20 text-xs" placeholder="Write something nice..." />
                                </div>
                                <div>
                                    <label className="block text-[#d7ccc8] text-[10px] uppercase tracking-widest font-sans mb-1">Song Recommendation (Optional)</label>
                                    <input type="text" value={song} onChange={(e) => setSong(e.target.value)} className="w-full bg-[#FDF1F3] text-ink px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#c88d9a] font-sans text-xs" placeholder="Song title or Spotify link" />
                                </div>
                                <div>
